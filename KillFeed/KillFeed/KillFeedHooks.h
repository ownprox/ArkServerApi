...
void NotifyDeath(const FString& KillerName, const FString& KillerTribe, const FString& VictimName, const FString& VictimTribe, const FString& WeaponName, UTexture2D* WeaponIcon, float Distance, bool IsPlayer)
{
	switch (DisplayType)
	{
	case 0:
		ArkApi::GetApiUtils().SendNotificationToAll(DisplayColour, DisplayScale, DisplayDelay, WeaponIcon, (IsPlayer ? Messages[0].c_str() : Messages[1].c_str()), *(KillerTribe.IsEmpty()  ? FString("") : FString("[" + KillerTribe + "]")), *KillerName, *(VictimTribe.IsEmpty() ? FString("") : FString("[" + VictimTribe + "]")), *VictimName, *WeaponName, trunc_decs(Distance, 2));
		break;
	case 1:
		ArkApi::GetApiUtils().SendNotificationToAll(DisplayColour, DisplayScale, DisplayDelay, WeaponIcon, (IsPlayer ? Messages[2].c_str() : Messages[3].c_str()), *(KillerTribe.IsEmpty() ? FString("") : FString("[" + KillerTribe + "]")), *KillerName, *(VictimTribe.IsEmpty() ? FString("") : FString("[" + VictimTribe + "]")), *VictimName, trunc_decs(Distance, 2));
		break;
	case 2:
		ArkApi::GetApiUtils().SendNotificationToAll(DisplayColour, DisplayScale, DisplayDelay, WeaponIcon, (IsPlayer ? Messages[0].c_str() : Messages[1].c_str()), *(KillerTribe.IsEmpty() ? FString("") : FString("[" + KillerTribe + "]")), *KillerName, *(VictimTribe.IsEmpty() ? FString("") : FString("[" + VictimTribe + "]")), *VictimName, trunc_decs(Distance, 2));
		break;
	}
	if (DisplayInChatbox) ArkApi::GetApiUtils().SendServerMessageToAll(DisplayColour, (IsPlayer ? Messages[0].c_str() : Messages[1].c_str()), *(KillerTribe.IsEmpty() ? FString("") : FString("[" + KillerTribe + "]")), *KillerName, *(VictimTribe.IsEmpty() ? FString("") : FString("[" + VictimTribe + "]")), *VictimName, *WeaponName, trunc_decs(Distance, 2));
}
...
{
	if (!Killer->IsLocalController() && Killer->IsA(AShooterPlayerController::StaticClass()))
	{
...
bool _cdecl Hook_APrimalDinoCharacter_Die(APrimalDinoCharacter* Dino, float KillingDamage, FDamageEvent* DamageEvent, AController* Killer, AActor* DamageCauser)
{
	if (Dino->TamingTeamIDField()() != 0 && !Killer->IsLocalController() && Killer->IsA(AShooterPlayerController::StaticClass()))
	{
		...
		if (KillerShooterController && KillerShooterController->GetPlayerCharacter())
		{
			FString DinoName, WeaponName = "Punch";
			UTexture2D* WeaponIcon = nullptr;
			...
			NotifyDeath(ArkApi::GetApiUtils().GetCharacterName(KillerShooterController), KillerShooterController->GetPlayerCharacter()->TribeNameField()(), DinoName, Dino->TribeNameField()(), WeaponName, WeaponIcon, Distance, false);
		}
	}
