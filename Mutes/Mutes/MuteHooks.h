...
void InitHooks()
{
	...
	ArkApi::GetCommands().AddOnTimerCallback("MutesSyncAndClean", &SyncAndClean);
}
...
void RemoveHooks()
{
	...
	ArkApi::GetCommands().RemoveOnTimerCallback("MutesSyncAndClean");
}

std::vector<MuteData>::iterator MuteItr;
int SecCounter = 0, SecCounter2 = 0;
DWORD MuteTimePasted;
void SyncAndClean()
{
	
	if (SecCounter2++ >= SyncSeconds)
	{
		SecCounter2 = 0;
		SyncMutes();
	}
...
bool _cdecl Hook_AShooterGameMode_HandleNewPlayer(AShooterGameMode* _this, AShooterPlayerController* NewPlayer, UPrimalPlayerData* PlayerData, AShooterCharacter* PlayerCharacter, bool bIsFromLogin)
{
	if (NewPlayer)
	{
		...
		if (Muted != 0)
		{
			const int64 hours = (Muted > 3600 ? (Muted / 3600) : 0), mins((hours > 0 && (Muted - (hours * 3600)) > 60) ? ((Muted - (hours * 3600)) / 60) : (Muted > 60 ? (Muted / 60) : 0));
			FString BanMsg;
		...
		if (BadWord.Num() > 0)
		{
			...
			if (Itr)
			{
				if (Itr->Minutes > 0)
				{
					...
					Log::GetLog()->info("Muted: {}, Word: {}, Minutes: {}", ArkApi::GetApiUtils().GetCharacterName(_this).ToString(), Itr->Word.ToString(), Itr->Minutes);
