...
void ChangeDinoColour(AShooterPlayerController* player, FString* message, int mode)
{
	...
	catch (...) { ArkApi::GetApiUtils().SendServerMessage(player, FLinearColor(255, 0, 0), "Incorrect Syntax: /sdc <Region> <Colour>"); }
	...
	if (Actor && Actor->IsA(APrimalDinoCharacter::GetPrivateStaticClass()))
	{
		...
		if (Dino && !Dino->IsDead())
		{
			...
			{
				if (Dino->OwningPlayerIDField()() != 0 && Dino->OwningPlayerIDField()() != player->LinkedPlayerIDField()() || !Dino->TribeNameField()().Equals(player->GetPlayerCharacter()->TribeNameField()()))
				{
	...
	else ArkApi::GetApiUtils().SendServerMessage(player, FLinearColor(255, 0, 0), "Please face the middle of your screen towards the dino you want to colour.");
}
